...
var SPECIAL_ATRULES = ['@page', '@font-face', '@viewport']; // Global variables (to avoid passing them back and forth):
...
var parse = function parse(blob) {
  ...
  info = {};
  resetInfo(); // Initialize the ancestors:

  ancestors = []; // The whole stylesheet simulates the ruleset block of a dummy ATRULE:
...
var attemptConversionToRule = function attemptConversionToRule() {
  for (var _i = 0, _SPECIAL_ATRULES = SPECIAL_ATRULES; _i < _SPECIAL_ATRULES.length; _i++) {
    var specialAtrule = _SPECIAL_ATRULES[_i];
